---
name: desktop-control-mint
version: 1.7.0
description: >
  Control the Linux Mint desktop via PyAutoGUI MCP. Implements a
...
tags: [linux, mint, desktop, automation, pyautogui, mcp, mouse, keyboard]
metadata:
  clawdbot:
    emoji: 🖥️
    requires:
...
# Desktop control — Linux Mint (PyAutoGUI MCP)

Controls the Linux Mint desktop from OpenClaw via a PyAutoGUI MCP server
running locally. All commands run via `mcporter call` in a bash shell.
...
Check if the `mint` mcporter alias exists, and create it if missing:
```bash
mcporter list mint 2>&1 || mcporter config add mint --url "${DESKTOP_MCP_URL:-http://127.0.0.1:8765/mcp}" --allow-http --yes
```
...
| Find canvas bounds   | Pixel  | `mcporter call mint.pyautogui_pixel --args '{"x":960,"y":350}'` |
...
5. **Verify with pixel color:**
   ```bash
   mcporter call mint.pyautogui_pixel --args '{"x":500,"y":300}'
   ```
...
6. **Scan for boundaries.** To find canvas edges or UI boundaries,
   ...
   mcporter call mint.pyautogui_pixel --args '{"x":960,"y":300}'
   mcporter call mint.pyautogui_pixel --args '{"x":960,"y":350}'
   ```
7. **Check coordinates are on screen:**
   ```bash
   mcporter call mint.pyautogui_onScreen --args '{"x":500,"y":300}'
   ```
...
for y in 200 250 300 350 400; do
  mcporter call mint.pyautogui_pixel --args "{\"x\":960,\"y\":$y}"
done
...
for x in 400 450 500 550 600; do
  mcporter call mint.pyautogui_pixel --args "{\"x\":$x,\"y\":CANVAS_VMID}"
done
...
```bash
mcporter call mint.pyautogui_getActiveWindowTitle
```

### List all visible windows
```bash
mcporter call mint.pyautogui_getAllTitles
```

### Find windows by title (substring match)
```bash
mcporter call mint.pyautogui_getWindowsWithTitle --args '{"title":"Calculator"}'
```
Returns: `[{"left": 619, "top": 0, "width": 1309, "height": 1028}]`

### Find which window is at a screen coordinate
```bash
mcporter call mint.pyautogui_getWindowsAt --args '{"x":500,"y":300}'
```

### Get full window object (position, size, state)
```bash
mcporter call mint.pyautogui_getActiveWindow
mcporter call mint.pyautogui_getAllWindows
```
...
2. **Alt+Tab** to cycle windows:
   ```bash
   mcporter call mint.pyautogui_hotkey --args '{"keys":["alt","tab"]}'
   ```
3. **Click the window in the panel/taskbar** (identify position from screenshot)
4. **Launch the app** via Alt+F2 if it's not open:
   ```bash
   mcporter call mint.pyautogui_hotkey --args '{"keys":["alt","f2"]}'
   # wait 0.5s
   mcporter call mint.pyautogui_typewrite --args '{"message":"gnome-calculator"}'
...
mcporter call mint.pyautogui_click --args '{"x":500,"y":300}'

# Right click
mcporter call mint.pyautogui_rightClick --args '{"x":500,"y":300}'

# Double click
mcporter call mint.pyautogui_doubleClick --args '{"x":500,"y":300}'

# Triple click (select entire line of text)
mcporter call mint.pyautogui_tripleClick --args '{"x":500,"y":300}'

# Middle click
mcporter call mint.pyautogui_middleClick --args '{"x":500,"y":300}'

# Multiple clicks with interval
mcporter call mint.pyautogui_click --args '{"x":500,"y":300,"clicks":3,"interval":0.1}'
```

### Move (no click)
```bash
# Absolute position
mcporter call mint.pyautogui_moveTo --args '{"x":500,"y":300}'
mcporter call mint.pyautogui_moveTo --args '{"x":500,"y":300,"duration":0.3}'

# Relative offset from current position
mcporter call mint.pyautogui_move --args '{"xOffset":100,"yOffset":-50}'
mcporter call mint.pyautogui_moveRel --args '{"xOffset":100,"yOffset":-50,"duration":0.2}'
```

### Get current mouse position
```bash
mcporter call mint.pyautogui_position
```

### Scroll
```bash
# Vertical scroll (positive=up, negative=down)
mcporter call mint.pyautogui_scroll --args '{"clicks":3}'
mcporter call mint.pyautogui_scroll --args '{"clicks":-3,"x":500,"y":300}'

# Horizontal scroll (positive=right, negative=left)
mcporter call mint.pyautogui_hscroll --args '{"clicks":3}'
mcporter call mint.pyautogui_hscroll --args '{"clicks":-3,"x":500,"y":300}'
```
...
mcporter call mint.pyautogui_moveTo --args '{"x":500,"y":300}'
mcporter call mint.pyautogui_dragTo --args '{"x":800,"y":400,"duration":0.5,"button":"left"}'
```

### Relative drag (by offset)
```bash
mcporter call mint.pyautogui_drag --args '{"xOffset":200,"yOffset":100,"duration":0.5}'
mcporter call mint.pyautogui_dragRel --args '{"xOffset":200,"yOffset":100,"duration":0.5}'
```
...
```bash
# Move to arc start
mcporter call mint.pyautogui_moveTo --args '{"x":400,"y":300}'

# Chain dragTo segments — each starts where the previous ended
mcporter call mint.pyautogui_dragTo --args '{"x":430,"y":330,"duration":0.2,"button":"left"}'
mcporter call mint.pyautogui_dragTo --args '{"x":465,"y":340,"duration":0.2,"button":"left"}'
mcporter call mint.pyautogui_dragTo --args '{"x":500,"y":330,"duration":0.2,"button":"left"}'
mcporter call mint.pyautogui_dragTo --args '{"x":530,"y":300,"duration":0.2,"button":"left"}'
```
...
mcporter call mint.pyautogui_typewrite --args '{"message":"Hello World"}'
mcporter call mint.pyautogui_typewrite --args '{"message":"slow typing","interval":0.05}'

# write is an alias for typewrite
mcporter call mint.pyautogui_write --args '{"message":"Hello World"}'
```
...
mcporter call mint.pyautogui_press --args '{"keys":["f5"]}'

# Press a key multiple times
mcporter call mint.pyautogui_press --args '{"keys":["tab"],"presses":3,"interval":0.1}'
```

### Hotkey combinations (simultaneous keys)
```bash
mcporter call mint.pyautogui_hotkey --args '{"keys":["ctrl","c"]}'
mcporter call mint.pyautogui_hotkey --args '{"keys":["ctrl","v"]}'
mcporter call mint.pyautogui_hotkey --args '{"keys":["ctrl","z"]}'
mcporter call mint.pyautogui_hotkey --args '{"keys":["ctrl","shift","s"]}'
mcporter call mint.pyautogui_hotkey --args '{"keys":["alt","f4"]}'
mcporter call mint.pyautogui_hotkey --args '{"keys":["alt","tab"]}'
mcporter call mint.pyautogui_hotkey --args '{"keys":["win","r"]}'
```

### Hold keys (within a single call)
```bash
# hold keeps keys pressed for the duration of the call
mcporter call mint.pyautogui_hold --args '{"keys":["shift"]}'
```
...
Validate a key name:
```bash
mcporter call mint.pyautogui_isValidKey --args '{"key":"enter"}'
```
...
```bash
mcporter call mint.pyautogui_sleep --args '{"seconds":1}'
```
...
Common app commands: `gnome-calculator`, `gedit`, `nemo`, `firefox`,
`gnome-terminal`, `xed`, `gimp`.

### Via menu
```bash
# Click the Mint menu (typically at bottom-left or top-left)
# Then type to search
mcporter call mint.pyautogui_typewrite --args '{"message":"calculator"}'
sleep 1
mcporter call mint.pyautogui_press --args '{"keys":["enter"]}'
```
...
mcporter call mint.pyautogui_diagnose

# List registered tools on the server
mcporter call mint.pyautogui_tools

# System info (PyAutoGUI version, screen size, etc.)
mcporter call mint.pyautogui_getInfo
...
mcporter call mint.pyautogui_pixelMatchesColor --args '{"x":500,"y":300,"expectedRGBColor":[255,255,255],"tolerance":10}'
```
